...
public class VideoRulerView extends View {
    ...
    long downTimestamp; // 记录缩放时按下的时间戳
...
//        setBackgroundResource(R.drawable.bg_wheel);
        ...
        shadowPaint.setColor(Color.parseColor("#3300ff7f"));
    ...
    public void drawScaleLine(Canvas canvas) {
        ...
        for (int i = 0; drawCount < width * 2; i++) {
            ...
            if ((mValue+i) == getmValue(maxTimestamp)){
                canvas.drawLine(xPosition, 0, xPosition, mDensity * ITEM_MAX_HEIGHT, linePaint);
                canvas.drawText("24:00", countLeftStart(mValue + i, xPosition, textWidth), getHeight() - textWidth, textPaint);
            }
            ...
            xPosition = (mWidth / 2 + mMove) - i * mDensity * (mLineDivider);
            if ((mValue-i) == getmValue(minTimestamp)){
                canvas.drawLine(xPosition, 0, xPosition, mDensity * ITEM_MAX_HEIGHT, linePaint);
                canvas.drawText("00:00", countLeftStart(mValue + i, xPosition, textWidth), getHeight() - textWidth, textPaint);
            }
            if ((mValue-i) > getmValue(minTimestamp)) {
                if (((mValue - i)) % largeScaleValue == 0) {
                    canvas.drawLine(xPosition, 0, xPosition, mDensity * ITEM_MAX_HEIGHT, linePaint);
        ...
        switch (event.getActionMasked() & event.getAction()) {
            case MotionEvent.ACTION_POINTER_DOWN:
                if (event.getPointerCount() == 2) {
                    ...
                    isMoving = false;
                    isScaling = true;
                    downTimestamp = getTimeInMillis(mValue);
                }
                break;
